...
export default function EditBookingModal({ booking, onClose, onSubmit }: EditBookingModalProps) {
  const [selectedClient, setSelectedClient] = useState<Client | null>({
    id: booking.clientId,
    name: booking.clientName,
    phone: booking.clientPhone,
  })
  const [callType, setCallType] = useState<"onboarding" | "follow-up">(booking.callType)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const endTime = calculateEndTime(booking.startTime, callType)
    ...
    if (callType === "follow-up") {
      bookingData.recurringStartDate = booking.date
    }
      ...
      <div className="bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl w-full max-w-lg mx-4 border border-white/20 mt-8 mb-8">
        <div className="flex items-center justify-between p-6 border-b border-gray-200/50 bg-gradient-to-r from-blue-50 to-purple-50 rounded-t-2xl">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Update your coaching session</p>
          </div>
          <button 
            onClick={onClose} 
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-xl border border-blue-200/50">
              ...
              <p className="text-sm text-gray-600 mt-1">Time cannot be changed</p>
            </div>
          </div>
